...
//     if(valueChanged){
//       flow.update(leds, *effects.getList("FLOW"));
...
// void Led::Flow::init(List *flow){
//   speed = flow->getValue("SPEED")->getValue(0);
//   amount = flow->getValue("AMOUNT")->getValue(0);
//   colorHSV = flow->getValue("COLOUR")->getValue(0);
// }

// void Led::Flow::update(CRGB* leds){
//   if(millis() - startTime > 1000 / speed){ //  1000 / speed = time in millisec for 1 px step
//     int change = round((millis() - startTime) * speed / 1000.0); // rounded steps in px
//     startTime = millis();
//     start += change;
//     if(start >= LED_NUM) start -= LED_NUM;
...
//     for(int i = amount - change; i < amount; i++){
//       if(start + i >= LED_NUM)
...
// void Led::Flow::update(CRGB* leds, List& flow){
//   amount = flow.getValue("AMOUNT")->getValue(0);
//   colorHSV = flow.getValue("COLOUR")->getValue(0);

//   if(flow.getValue("SPEED")->getValue(0) != 0)
//     speed = flow.getValue("SPEED")->getValue(0);
